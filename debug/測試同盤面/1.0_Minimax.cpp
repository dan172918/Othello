...
int Minimax(int checkerboard[][Size],int black,int white,int player,int depth,Node sn,Node en,bool skip){
	if(depth==0||black+white==64||white==0||black==0){
		...
		if(arr.size()==0){
			if(skip)
				return (!player)?black-white:white-black;
			return -1*Minimax(checkerboard,black,white,!player,depth,sn,en,true);	
		}
		nodetotal+=arr.size();
...
int main(){
	LogCheckerboard.open("./1.0_Minimax.csv",ios::out);
	LogCheckerboard<<"depth "+to_string(limitDep)+" :"<<endl;
	LogCheckerboard<<"Time,Node"<<endl;
//	LogCheckerboard.open("./1.0_minimax_PV.txt",ios::out);
