...
//			pvLog<<"pare : ("<<test.row<<","<<test.col<<") depth : "<<depth-1<<" ( "<<arr[i].row<<","<<arr[i].col<<" ) : "<<-1*t<<endl;
			if(t>m){
				m=t;
				if(depth==limitDep){
					ans=arr[i];	
