...
void outputCheckerboard(int checkerboard[][Size],int black,int white,int player){
	LogCheckerboard<<"    ";
...
bool isGameOver(int checkerboard[][Size],int black,int white,Node sn,Node en){
	if(black+white==(Size-1)*(Size-1)||white==0||black==0||!(checkCanPlayChess(checkerboard,0,sn,en)||checkCanPlayChess(checkerboard,1,sn,en)))
		return true;
	else
		return false;
}
	...
	if(depth==0||isGameOver(checkerboard,black,white,sn,en)){
		...
		if(arr.size()==0){
			...
			for(int i=0;i<arr.size();i++){
				...
				t=-1*Minimax(temp,tempB,tempW,!player,depth-1,tempSn,tempEn);
				if(t>m)
					m=t;
			}	
		}
...
int main(){
	LogCheckerboard.open("./Minimax.csv",ios::out);
	LogCheckerboard<<"depth 2 :,,depth 4 :,,depth 6 :,,depth 8 :,,"<<endl;
	LogCheckerboard<<"Time,Node,Time,Node,Time,Node,Time,Node"<<endl;
	...
	for(int i=1;i<=15;i++){
		...
		for(limitDep=3;limitDep<10;limitDep+=2){
