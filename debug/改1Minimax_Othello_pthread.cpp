...
struct parameter{
	...
	Node sn,en,pare;
	parameter(int temp[][Size],int b,int w,int p,int d,Node S,Node E,Node t){
	...
	if(par.depth==0||isGameOver(par.checkerboard,par.black,par.white,par.sn,par.en)){
		...
		for(int i=0;i<arr.size();i++){
			...
			if(!samePlayer)
				*(int*)&arr[i].v*=-1;
//			pthread_mutex_lock(&count_mutex);
//			pvLog<<"depth : "<<par.depth<<" ( "<<arr[i].row<<","<<arr[i].col<<" ) : "<<*(int*)&arr[i].v<<endl;
//			pthread_mutex_unlock(&count_mutex);
			if(*(int*)&arr[i].v>m)
				m=*(int*)&arr[i].v;
		}
	...
	if(par.depth==0||isGameOver(par.checkerboard,par.black,par.white,par.sn,par.en))
		...
		for(int i=0;i<arr.size();i++){
			...
			if(par.depth==limitDep){
				pvLog<<"pare : ("<<par.pare.row<<","<<par.pare.col<<") depth : "<<par.depth<<" ( "<<arr[i].row<<","<<arr[i].col<<" ) : "<<*(int*)&arr[i].v<<endl;
				cout<<"pare : ("<<par.pare.row<<","<<par.pare.col<<") depth : "<<par.depth<<" ( "<<arr[i].row<<","<<arr[i].col<<" ) : "<<*(int*)&arr[i].v<<endl;
			} 
			pthread_mutex_unlock(&count_mutex);
			if(*(int*)&arr[i].v>m){
				m=*(int*)&arr[i].v;
...
void AIChess(int checkerboard[][Size]){	
	...
	nodetotal=0;
	parameter par(checkerboard,black,white,0,limitDep,sn,en,ans);
	pthread_t id;
	START = clock();
...
int main(){
	file1.open("./test/"+to_string(limitDep-1)+"/改11.0pthread.csv",ios::out);
	...
	pvLog.open("./test/"+to_string(limitDep-1)+"/tryPV5.txt",ios::out);
