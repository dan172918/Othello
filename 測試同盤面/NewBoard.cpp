...
fstream createBoard,storeboard;
...
struct boardLog{
	int checkerboard[Size][Size],player;
	boardLog *pv;
	boardLog(){}
};

void printCheckerboard(int checkerboard[][Size],int p){
//	system("cls");
	cout<<"    ";
...
int AlphaBeta(int checkerboard[][Size],int alpha,int beta,int black,int white,int player,int depth,Node sn,Node en,bool skip,boardLog* &board){
	if(depth==limitDep||black+white==64||white==0||black==0){
		...
		if(arr.size()==0){
			if(skip)
				return (!player)?black-white:white-black;
			return -1*AlphaBeta(checkerboard,-1*beta,-1*alpha,black,white,!player,depth,sn,en,true,board);
		}
		if(depth<6){
		...
		for(int i=0;i<arr.size();i++){
			...
			boardLog *tempboard = new boardLog();
			memcpy(tempboard->checkerboard,temp,sizeof(tempboard->checkerboard));
			tempboard->player=!player;
			updateNode(temp,tempSn,tempEn,arr[i].row,arr[i].col);
			int t=-1*AlphaBeta(temp,-1*beta,-1*Max(m,alpha),tempB,tempW,!player,depth+1,tempSn,tempEn,false,tempboard->pv);
			if(t>m){
				board=tempboard;
				m=t;
			}
...
void AIChess(int checkerboard[][Size],int index){
	int temp[Size][Size];
	boardLog *board = new boardLog();
	memcpy(board->checkerboard,checkerboard,sizeof(board->checkerboard));
	board->player=player;
	memcpy(temp,checkerboard,sizeof(temp));
	int a=AlphaBeta(temp,-1000,1000,black,white,player,0,sn,en,false,board->pv);
	board=board->pv;
	int num=black+white+1;
	while(board){
		storeboard.open("./board/"+to_string(num)+"/"+to_string(index)+".txt",ios::out);
		if(board->player)
			storeboard<<"o";
		else
			storeboard<<"x";
...
int main(){
	...
	string str;
	for(int folder=40;folder<50;folder+=10){
		for(int i=0;i<25;i++){
			black=0;white=0;
			...
			createBoard.open("./board/"+to_string(folder)+"/"+to_string(boardarr[i])+".txt",ios::in);
			createBoard>>str;
			player = (str=="o")?1:0;
			for(int j=1;j<Size;j++){
				createBoard>>str;
			...
			AIChess(checkerboard,boardarr[i]);
		}		
	}	
}
