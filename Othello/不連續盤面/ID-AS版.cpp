...
void UpdateBoard(int board[][Size],Node node,int &black,int &white,int player){
	int xStart,yStart;
...
void AI(int board[][Size]){
	...
	int best=AlphaBeta(temp,-10000,10000,black,white,player,2,sn,en,false),m;
	LogBoard<<","<<(double)(clock() - start) / CLOCKS_PER_SEC<<","<<nodetotal;
	
	for(limitDep=4 ; limitDep<=12; limitDep+=2){
		nodetotal = 0;
		m=AlphaBeta(temp,best-3,best+3,black,white,player,limitDep,sn,en,false);
		
		if(m<=best-3){
			m=AlphaBeta(temp,-10000,m,black,white,player,limitDep,sn,en,false);
		}
		else if(m>=best+3){
			m=AlphaBeta(temp,m,10000,black,white,player,limitDep,sn,en,false);
		}
...
int main(){
	LogBoard.open("./ID-AS版.csv",ios::out);
