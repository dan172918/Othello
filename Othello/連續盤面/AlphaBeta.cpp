...
int black=2, white=2, player=1, nodetotal=0;
...
void AI(int board[][Size]){
	...
	memcpy(temp, board, sizeof(temp));
	START = clock();
	AlphaBeta(temp, -1000, 1000, black, white, player, limitDep, sn, en, false);
	END = clock();
...
int main(){
	LogBoard.open("./Alpha-beta.csv",ios::out);
	LogBoard<<"depth "+to_string(limitDep)+" :"<<endl;
	LogBoard<<"Time,Node"<<endl;
	HistoryBoard.open("./HisBoard.txt",ios::out);
