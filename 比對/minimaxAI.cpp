...
int minimax(int checkerboard[][Size],int black,int white,int player,int depth,Node* &node){
	int m;
	if(depth==0||isGameOver(checkerboard,player,black,white)){
		...
		if(arr.size()==0)
			m=-1*minimax(checkerboard,black,white,!player,depth,node);
		else{
			m=INT_MIN;
			for(int i=0;i<arr.size();i++){
				...
				int t=-1*minimax(temp,tempB,tempW,!player,depth-1,arr[i]->next);
				if(depth==limitDep)
					t*=-1;				
				if(t>m){
					m=t;
...
int main(){
	...
	for(int i=1;i<=400;i++){
		...
		Node *root = new Node();
		file2<<i<<" : "<<endl;
		minimax(checkerboard,black,white,player,limitDep,root->next);
		while(1){
			root = root->next;
