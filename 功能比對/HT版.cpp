...
int AlphaBeta(int OthelloBoard[][Size],int alpha,int beta,int black,int white,int player,int depth,Node sn,Node en,bool skip){
	if(/*timeIsUp()||*/depth==0||black+white==64||white==0||black==0){
...
//					num++;					
//				}
				Priority[arr[best].row][arr[best].col]+=pow(2,depth-1);
				return m;
			}				
		}
		Priority[arr[best].row][arr[best].col]+=pow(2,depth-1);
...
//		cout<<"E"<<endl;
		nodetotal = 0;
		m=AlphaBeta(temp,-10000,10000,black,white,player,limitDep,sn,en,false);
...
int main(){
	LogOthelloBoard.open("./HTª©_Alpha-Beta.csv",ios::out);
