...
void updateOthelloBoard(int OthelloBoard[][Size],Node node,int &black,int &white,int player){
	int xStart,yStart;
	for(int i=node.dir;i!=8;i/=10){
		int nowdir=i%10;
		xStart=node.row+dir[nowdir][0];yStart=node.col+dir[nowdir][1];
	...
	for(int i=0;i<8;i++){
		...
		if(OthelloBoard[xStart][yStart]>=0){
			OthelloBoard[xStart][yStart] = OthelloBoard[xStart][yStart]*10 + (i+4)%8;		
		}
	}
}
	...
	for(int i=sn.row;i<=en.row;i++){
		for(int j=sn.col;j<=en.col;j++){
			...
			if(OthelloBoard[i][j]<0||OthelloBoard[i][j]==8)
				continue;
			for(int k=OthelloBoard[i][j];k!=8;k/=10){
				int nowdir=k%10;
				int xStart=i+dir[nowdir][0],yStart=j+dir[nowdir][1];
...
//					cout<<setw(9)<<"o";
//				else if(OthelloBoard[i][j]==8)
//					cout<<setw(9)<<"_";
//				else
//					cout<<setw(9)<<OthelloBoard[i][j];		
//			}
//			cout<<endl;
//		}
//		for(auto i:arr){
...
int main(){
	LogOthelloBoard.open("./зя2_Alpha-Beta.csv",ios::out);
				...
				if(OthelloBoard[a][j]<0){
					for(int k=0;k<8;k++){
						if(a+dir[k][0]>0&&a+dir[k][0]<Size&&j+dir[k][1]>0&&j+dir[k][1]<Size&&OthelloBoard[a+dir[k][0]][j+dir[k][1]]>=0){
							OthelloBoard[a+dir[k][0]][j+dir[k][1]] = OthelloBoard[a+dir[k][0]][j+dir[k][1]]*10 + (k+4)%8;
						} 
					}					
				}
			}
