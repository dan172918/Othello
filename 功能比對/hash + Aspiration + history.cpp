...
int black=2,white=2,player=1,limitDep,MaxNum=36;
...
int PriorityPlayer[Size][Size]={{0,1,2,3,4,5,6,7,8},
						  {1,0,7,1,3,3,1,7,0},
						  {2,7,8,4,5,5,4,8,7},
						  {3,1,4,2,6,6,2,4,1},
					 	  {4,3,5,6,0,0,6,5,3},
...
//			if(timeIsUp())return m;
		...
		Priority[arr[best].row][arr[best].col]+=pow(2,limitDep-depth-1);
		update_hash(hash_val,hash_index,NeedUpdate,limitDep-depth,(m>alpha)?0:1,m,bestMove);
		return m;
	}
}
...
void AIChess(int OthelloBoard[][Size],uint128_t &hash_val){
	...
	nodetotal = 0;
	limitDep=2;
	int best=AlphaBeta(temp,-10000,10000,black,white,player,0,sn,en,false,hash_val);
	LogOthelloBoard<<","<<(double)(clock() - start) / CLOCKS_PER_SEC<<","<<nodetotal<<","<<best;
	for(limitDep=4 ; limitDep<=12; limitDep+=2){
		nodetotal = 0;
		m=AlphaBeta(temp,best-3,best+3,black,white,player,0,sn,en,false,hash_val);
//		counter_aging();
		if(m<=best-3){
			m=AlphaBeta(temp,-10000,m,black,white,player,0,sn,en,false,hash_val);
//			counter_aging();
		}
		else if(m>=best+3){
			m=AlphaBeta(temp,m,10000,black,white,player,0,sn,en,false,hash_val);
//			counter_aging();
		}
		best=m;
...
bool methodPlayer(Node a,Node b){
	if(PriorityPlayer[a.row][a.col]<PriorityPlayer[b.row][b.col])
		return true;
	return false;
}

int num=0;
int playerAddress[2][30];
void playerChess(int OthelloBoard[][Size],uint128_t &hash_val){
	vector<Node> arr = findnode(OthelloBoard,player,sn,en);
	sort(arr.begin(),arr.end(),methodPlayer);	
...
//	int temp[2][30]={{4, 2, 6, 6, 3, 5, 4, 6, 4, 2, 7, 6, 8, 6, 8, 8, 4, 2, 2, 1, 4, 5, 3, 1, 1, 6, 6, 2, 7, 8 },
//					 {6, 6, 6, 8, 7, 7, 8, 4, 7, 5, 4, 5, 3, 3, 6, 5, 3, 3, 4, 5, 2, 2, 1, 1, 7, 1, 2, 7, 7, 1 }};
...
//	for(auto i:arr){
//		cout<<"( "<<i.row<<","<<i.col<<" ) ";
//		if(PriorityPlayer[i.row][i.col]==PriorityPlayer[arr[0].row][arr[0].col])
//			temp++;
//	}
//	temp=rand()%temp;
//	playerAddress[0][num]=arr[temp].row;
//	playerAddress[1][num]=arr[temp].col;
//	num++;
//	//update board
//	updateOthelloBoard(OthelloBoard,arr[temp],black,white,player,hash_val);
//	//update move generator range
//	updateNode(OthelloBoard,sn,en,arr[temp].row,arr[temp].col);
//	player = 0;
//	//update print board message
//	printOthelloBoard(OthelloBoard);
//	return;	
...
int main(){
	srand(time(0));
	LogOthelloBoard.open("./hash + Aspiration + history.csv",ios::out);
//	file1.open("./player.txt",ios::out);
