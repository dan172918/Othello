...
//	}
	...
	nodetotal = 0;
	limitDep=2;
	int best=AlphaBeta(temp,-10000,10000,black,white,player,0,sn,en,false),m,threshould = 3;
	LogOthelloBoard<<","<<(double)(clock() - start) / CLOCKS_PER_SEC<<","<<nodetotal<<","<<best;
	for(limitDep=4 ; limitDep<=14; limitDep+=2){
		nodetotal = 0;
...
int main(){
	LogOthelloBoard.open("./°ÊºA°Ï¶¡ª©_Alpha-Beta.csv",ios::out);
//	LogOthelloBoard<<"board,U/D,depth,Node"<<endl;

//	LogOthelloBoard<<",depth2,,depth4,,depth6,,depth8,,depth10,,depth12,,depth14,,"<<endl;
//	LogOthelloBoard<<"board,Time,Node,Time,Node,Time,Node,Time,Node,Time,Node,Time,Node,Time,Node"<<endl;
