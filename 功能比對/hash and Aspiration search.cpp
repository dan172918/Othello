...
//	i++;
	...
	for(limitDep=4 ; limitDep<=12; limitDep+=2){
		...
		if(m<=best-3){
			m=AlphaBeta(temp,best-3-4,m,black,white,player,0,sn,en,false,hash_val);
			if(m<=best-3-4)
				m=AlphaBeta(temp,-10000,m,black,white,player,0,sn,en,false,hash_val);
		}
		else if(m>=best+3){
			m=AlphaBeta(temp,m,best+3+4,black,white,player,0,sn,en,false,hash_val);
			if(m>=best+3+4)
...
int main(){
	srand(time(0));
	LogOthelloBoard.open("./Aspiration_search_hash-PK.csv",ios::out);
